...
public class FallingSlabEntity extends FallingBlockEntity {
	...
	public FallingSlabEntity(World level, double x, double y, double z, BlockState state) {
		super(level, x, y, z, state);
	}

	/**
	 * This is copied from super class and modified to my needs.
	 * I want my slab to drop 2 item if was a double slab.
	 * There is unfortunately no other way.
	 */
	public void tick() {
		if (this.blockState.isAir(this.level, this.getOnPos())) {
			this.remove();
		} else {
						...
						if (!this.cancelDrop) {
							...
							if (flag2 && flag4) {
								...
								if (this.level.setBlock(blockpos1, this.blockState, 3)) {
									...
									if (blockState.getValue(FallingSlab.TYPE) == SlabType.DOUBLE) {
										this.spawnAtLocation(block);
									}
									this.spawnAtLocation(block);
								}
