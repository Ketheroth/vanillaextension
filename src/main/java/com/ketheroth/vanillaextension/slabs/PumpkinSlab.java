	...
	public InteractionResult use(BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit) {
		...
		if (itemstack.canPerformAction(ToolActions.SHEARS_CARVE)) {
			if (!worldIn.isClientSide) {
				...
				worldIn.setBlock(pos, VEBlocks.carved_pumpkin_slab.get().defaultBlockState().setValue(TYPE, state.getValue(TYPE)).setValue(WATERLOGGED, state.getValue(WATERLOGGED)), 11);
