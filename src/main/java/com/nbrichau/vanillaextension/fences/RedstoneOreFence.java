...
public class RedstoneOreFence extends FenceBlock {
	...
	public RedstoneOreFence(Properties properties) {
		super(properties);
		this.setDefaultState(this.stateContainer.getBaseState().with(NORTH, Boolean.FALSE).with(EAST, Boolean.FALSE).with(SOUTH, Boolean.FALSE).with(WEST, Boolean.FALSE).with(WATERLOGGED, Boolean.FALSE).with(LIT, Boolean.FALSE));
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
		...
		if (player.getHeldItem(handIn).getItem() == Items.LEAD) {
			if (worldIn.isRemote) {
