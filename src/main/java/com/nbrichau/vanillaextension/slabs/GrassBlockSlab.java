...
public class GrassBlockSlab extends SlabBlock implements IGrowable {
	...
	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
		if (!isSnowyConditions(state, worldIn, pos)) {
			...
			worldIn.setBlockState(pos, BlockInit.dirt_slab.getDefaultState().with(TYPE, state.get(TYPE)).with(WATERLOGGED, state.get(WATERLOGGED)));
		} else {
				...
				for (int i = 0; i < 4; ++i) {
					...
					if (isSnowyAndNotUnderwater(blockstate, worldIn, blockpos)) {
						if (blockstate.isIn(Blocks.DIRT)) {
							...
							worldIn.setBlockState(blockpos, BlockInit.grass_block_slab.getDefaultState().with(TYPE, blockstate.get(TYPE)).with(WATERLOGGED, blockstate.get(WATERLOGGED)));
