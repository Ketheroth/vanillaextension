...
public class ConcretePowderStairs extends ConcretePowderBlock implements IWaterLoggable{
	...
	public ConcretePowderStairs(Block solidified, Properties properties) {
		...
		this.setDefaultState(this.getStateContainer().getBaseState().with(FACING, Direction.NORTH).with(HALF, Half.BOTTOM).with(SHAPE, StairsShape.STRAIGHT).with(WATERLOGGED, Boolean.valueOf(false)));
	...
	public void onEndFalling(World worldIn, BlockPos pos, BlockState fallingState, BlockState hitState, FallingBlockEntity fallingBlock) {
		if (shouldSolidify(worldIn, pos, hitState)) {//getShapeProperty(blockstateSolidified, context.getWorld(), blockpos)
			worldIn.setBlockState(pos, this.solidifiedState.with(FACING, fallingState.get(FACING)).with(HALF, fallingState.get(HALF)).with(SHAPE, fallingState.get(SHAPE)).with(WATERLOGGED, fallingState.get(WATERLOGGED)), 3);
		}
	}
	...
	public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
		...
		if (isTouchingLiquid(worldIn, currentPos)) {
			return facing.getAxis().isHorizontal() ?
				this.solidifiedState.with(SHAPE, getShapeProperty(stateIn, worldIn, currentPos)) :
				super.updatePostPlacement(this.solidifiedState, facing, facingState, worldIn, currentPos, facingPos);
		} else {
			return facing.getAxis().isHorizontal() ?
				stateIn.with(SHAPE, getShapeProperty(stateIn, worldIn, currentPos)) :
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		BlockState blockstateSolidified = solidifiedState.with(FACING, context.getPlacementHorizontalFacing()).with(HALF, direction != Direction.DOWN && (direction == Direction.UP || !(context.getHitVec().y - (double) blockpos.getY() > 0.5D)) ? Half.BOTTOM : Half.TOP).with(WATERLOGGED, fluidstate.getFluid() == Fluids.WATER);
		BlockState blockstatePowder = this.getDefaultState().with(FACING, context.getPlacementHorizontalFacing()).with(HALF, direction != Direction.DOWN && (direction == Direction.UP || !(context.getHitVec().y - (double) blockpos.getY() > 0.5D)) ? Half.BOTTOM : Half.TOP).with(WATERLOGGED, fluidstate.getFluid() == Fluids.WATER);

		return shouldSolidify(iblockreader, blockpos, iblockreader.getBlockState(blockpos)) ?
			blockstateSolidified.with(SHAPE, getShapeProperty(blockstateSolidified, context.getWorld(), blockpos))
			: blockstatePowder.with(SHAPE, getShapeProperty(blockstatePowder, context.getWorld(), blockpos));
	}
