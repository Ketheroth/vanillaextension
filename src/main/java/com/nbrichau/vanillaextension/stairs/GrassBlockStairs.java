...
public class GrassBlockStairs extends StairsBlock {
	...
	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
		if (!isSnowyConditions(state, worldIn, pos)) {
			...
			worldIn.setBlockState(pos, BlockInit.dirt_stairs.getDefaultState().with(FACING, state.get(FACING)).with(HALF, state.get(HALF)).with(SHAPE, state.get(SHAPE)).with(WATERLOGGED, state.get(WATERLOGGED)));
		} else {
				...
				for (int i = 0; i < 4; ++i) {
					...
					if (isSnowyAndNotUnderwater(blockstate, worldIn, blockpos)) {
						if (blockstate.isIn(Blocks.DIRT)) {
							...
							worldIn.setBlockState(blockpos, BlockInit.grass_block_stairs.getDefaultState().with(FACING, blockstate.get(FACING)).with(HALF, blockstate.get(HALF)).with(SHAPE, blockstate.get(SHAPE)).with(WATERLOGGED, blockstate.get(WATERLOGGED)));
