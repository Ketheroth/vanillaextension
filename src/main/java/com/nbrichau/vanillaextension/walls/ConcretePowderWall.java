...
public class ConcretePowderWall extends ConcretePowderBlock implements IWaterLoggable {
	...
	public void onEndFalling(World worldIn, BlockPos pos, BlockState fallingState, BlockState hitState, FallingBlockEntity fallingBlock) {
		if (shouldSolidify(worldIn, pos, hitState)) {
			worldIn.setBlockState(pos, this.solidifiedState.with(UP, fallingState.get(UP)).with(WALL_HEIGHT_NORTH, fallingState.get(WALL_HEIGHT_NORTH)).with(WALL_HEIGHT_EAST, fallingState.get(WALL_HEIGHT_EAST)).with(WALL_HEIGHT_SOUTH, fallingState.get(WALL_HEIGHT_SOUTH)).with(WALL_HEIGHT_WEST, fallingState.get(WALL_HEIGHT_WEST)).with(WATERLOGGED, fallingState.get(WATERLOGGED)), 3);
		}
	}
